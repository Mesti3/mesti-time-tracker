...
import static com.GitIntegration.SetupCommitHookResult.GIT_DIR_NOT_FOUND;
import static com.GitIntegration.SetupCommitHookResult.GIT_HOOKS_DIR_NOT_FOUND;
 ...
 */
@State(name="MestiTimeTracker", storages = {@Storage(value = StoragePathMacros.WORKSPACE_FILE)})
public final class TimeTrackerComponent implements ProjectComponent, PersistentStateComponent<TimeTrackerPersistentState>, Disposable {
    ...
    private final ServerIntegration server = new ServerIntegration();

    private static final NotificationGroup NOTIFICATION_GROUP = new NotificationGroup("Mesti Time Tracker", NotificationDisplayType.BALLOON, false, null, EmptyIcon.ICON_0);

    private static final NotificationGroup IDLE_NOTIFICATION_GROUP = new NotificationGroup("Mesti Time Tracker - Idle time", NotificationDisplayType.BALLOON, true, null, EmptyIcon.ICON_0);
    ...
    public static final String AUTH = "NO AUTHENTICATE";
    public static final String TOKEN = "No token";
    ...
    @Nullable
    private UserPattern userPattern = new UserPattern("",null) ;
    @Nullable
    private TimePattern gitTimePattern;
    @Nullable
    private UserPattern gitUserPattern;
    ...
    public synchronized void toggleRunning() {
        ...
        switch (this.status) {
            case RUNNING:
                try{
                    server.sendPost(this.getEmail(),this.getTotalTimeSeconds(),project().getName());
                    setStatus(Status.STOPPED);
                }catch (Exception e){
                    System.out.format("%s",  e);
                }
                //here comes get request
                break;
            case STOPPED:
                try{
                  this.setEmail(server.sendGet(this.getToken()));
                }catch (Exception e){
                    System.out.format("%s",  e);
                }
    ...
    public String getEmail() {
      ...
      String email = this.userPattern.getEmail();
        return email == null ? EMAIL : email;
    }

    public synchronized void setEmail(@NotNull String pemail) {
        UserPattern user = this.userPattern;
    ...
    public String getToken() {
        ...
        String token = this.userPattern.getToken();
        return token == null ? TOKEN : token;
    }

    public synchronized void setToken(@NotNull String ptoken) {
        UserPattern user = this.userPattern;
        user.setToken(ptoken);
    }
    ...
    public String userAuth() {
        ...
        String auth;
       if(getToken().equals(TOKEN) || getToken().equals("")) {
           auth = this.userPattern.userAuth(this.getEmail(), this.getTotalTimeSeconds(), project().getName());
    ...
    @NotNull
    public UserPattern getGitUserPattern() {
        final UserPattern pattern = this.gitUserPattern;
        return pattern;// can change
    }

    public synchronized void setGitUserPattern(@NotNull UserPattern gitUserPattern) {
        this.gitUserPattern = gitUserPattern;
        updateGitUser("");
    }

    private void updateGitUser(String token) {
        final GitIntegration gitIntegrationComponent = this.gitIntegrationComponent;
        final UserPattern gitUserePattern = this.gitUserPattern;
        if (gitIntegration && gitIntegrationComponent != null && gitUserPattern != null) {
            gitIntegrationComponent.updateVersionUserFile(token, gitUserePattern);
        }
    }
    ...
    public void loadState(@NotNull TimeTrackerPersistentState state) {
        ...
        ApplicationManager.getApplication().invokeLater(() -> {
            synchronized (this) {
                ...
                setGitTimePattern(TimePattern.parse(state.gitTimePattern)); /////////////set for user
                setGitUserPattern(UserPattern.parse(state.gitUserPattern));
    ...
    public synchronized TimeTrackerPersistentState getState() {
        ...
        result.ideUserPattern = userPattern != null ? userPattern.getToken(): TimeTrackerPersistentState.DEFAULT_IDE_USER_PATTERN;
        result.gitUserPattern = gitUserPattern != null ? gitUserPattern.getToken() : TimeTrackerPersistentState.DEFAULT_GIT_USER_PATTERN;
        return result;
    }
